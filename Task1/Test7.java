...
public class Test7 {
    ...
    public static int getNumberOfToysInCartMsg(WebDriver driver) {
        List<WebElement> cartMessages = driver.findElement(By.className("cart-msg")).findElements(By.tagName("span"));
        return Integer.parseInt(cartMessages.get(1).getText().trim());
    }

    public static double getTotalCostInCartPage(WebDriver driver) {
        return Double.parseDouble(driver.findElement(By.className("total")).getText().trim().split(" ")[1]);
    }

    public static double getTotalCost(ArrayList<Toy> toys) {
        return toys.stream().map((toy) -> toy.getPrice()).reduce(0.0, (a, b) -> a + b);
    }
    ...
    public static void main(String[] args) {
        ...
        // CHECK IF CART MESSAGE COUNT IS EQUAL TO NUMBER OF TOYS BOUGHT
        if (getTotalCost(boughtToys) == getTotalCostInCartPage(driver)) {
